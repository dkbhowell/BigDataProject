...
public class ZillowBoroughsReducer extends Reducer<Text, Text, Text, Text>{
    
    private static final Logger logger = Logger.getLogger(ZillowBoroughsReducer.class.getName());
    
    @Override
    public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException{
        String borough = key.toString();
        //sum of %*size
        ...
        double sum_property_taxes;
        double sum_val_per_sqft;
        double sum_price_per_sqft;

        double avg_property_tax;
        double avg_sale_price_per_sqft;
        double avg_turnover;
        double average_home_size_per_borough;
        double average_year_built_per_borough;
        ...
        for (Text dataText : values){
            ...
            String zipCode = stringValues[0].trim();
    ...
    private String getAvgHomeSizePerZip(double sz1000, double sz3600, double sz1200, double sz1600, double sz2100) {
            ...
            if (numSizes > 0){
            double sum_percents = sz1000 + sz3600 + sz1200 + sz1600 + sz2100;
            double avgSize = sum_percents/num_sizes;
            }
            else{
                avgSize = -1;
            }
    ...
    private String getAvgYearBuiltPerZip(double s, double yr1900, double yr1930, double yr1950, double yr1970, double yr1990) {
            ...
            if(num_years > 0){
            double sum_percents = yr2000+ yr1900+ yr1930+ yr1950+ yr1970+ yr1990;
            double avg_yr = sum_percents/num_years;
            }
            else{
                avg_yr = -1;
            }
